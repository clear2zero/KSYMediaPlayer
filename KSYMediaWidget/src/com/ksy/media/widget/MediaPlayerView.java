...
public class MediaPlayerView extends RelativeLayout {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) throws IllegalArgumentException,NullPointerException {
		...
		this.mLockMode = typedArray.getBoolean(R.styleable.PlayerView_lockmode, false);
		typedArray.recycle();
		...
		this.mDeviceNaturalOrientationLandscape = (MediaPlayerUtils.getDeviceNaturalOrientation(mWindow) == MediaPlayerUtils.DEVICE_NATURAL_ORIENTATION_LANDSCAPE ? true: false);
		...
		this.mRootView = (ViewGroup) mLayoutInflater.inflate(R.layout.media_player_view, null);
		...
		this.mMediaPlayerCoverView = (MediaPlayerEventActionView) mRootView.findViewById(R.id.ks_camera_covering_view);
		this.mMediaPlayerControllerViewLarge = (MediaPlayerLargeControllerView) mRootView.findViewById(R.id.media_player_controller_view_large);
		this.mMediaPlayerControllerViewSmall = (MediaPlayerSmallControllerView) mRootView.findViewById(R.id.media_player_controller_view_small);
		...
		this.mMediaPlayerVideoView.setMediaPlayerController(mMediaPlayerController);
		RelativeLayout.LayoutParams MediaPlayerVideoViewParams = new LayoutParams(LayoutParams.WRAP_CONTENT,LayoutParams.WRAP_CONTENT);
		MediaPlayerVideoViewParams.addRule(RelativeLayout.CENTER_IN_PARENT);
		
		RelativeLayout.LayoutParams MediaPlayerBufferingViewParams = new LayoutParams(LayoutParams.WRAP_CONTENT,LayoutParams.WRAP_CONTENT);
		MediaPlayerBufferingViewParams.addRule(RelativeLayout.CENTER_IN_PARENT);
		this.mMediaPlayerBufferingView.hide();
		
		RelativeLayout.LayoutParams MediaPlayerLoadingViewParams = new LayoutParams(LayoutParams.MATCH_PARENT,LayoutParams.MATCH_PARENT);
		MediaPlayerLoadingViewParams.addRule(RelativeLayout.CENTER_IN_PARENT);
		this.mMediaPlayerLoadingView.hide();
		
		RelativeLayout.LayoutParams mediaPlayerCoverViewParams = new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
		mediaPlayerCoverViewParams.addRule(RelativeLayout.CENTER_IN_PARENT);
		this.mMediaPlayerCoverView.setCallback(new MediaPlayerEventActionView.EventActionViewCallback() {
...
//		    mMediaPlayerController.start();             
		...
		this.mMediaPlayerControllerViewLarge.setMediaPlayerController(mMediaPlayerController);
		this.mMediaPlayerControllerViewLarge.setHostWindow(mWindow);
		this.mMediaPlayerControllerViewLarge.setDeviceNavigationBarExist(mDeviceNavigationBarExist);
		this.mMediaPlayerControllerViewLarge.setNeedGestureDetector(mNeedGesture);
		this.mMediaPlayerControllerViewLarge.setNeedGestureAction(mNeedLightGesture, mNeedVolumeGesture, mNeedSeekGesture);
		...
		this.mMediaPlayerControllerViewSmall.setMediaPlayerController(mMediaPlayerController);
		this.mMediaPlayerControllerViewSmall.setHostWindow(mWindow);
		this.mMediaPlayerControllerViewSmall.setDeviceNavigationBarExist(mDeviceNavigationBarExist);
		this.mMediaPlayerControllerViewSmall.setNeedGestureDetector(true);
		this.mMediaPlayerControllerViewSmall.setNeedGestureAction(false, false, false);
		...
		if (MediaPlayerUtils.isFullScreenMode(mPlayMode)) {
		    addView(mMediaPlayerControllerViewLarge, mMediaPlayerControllerViewLargeParams);
		    mMediaPlayerControllerViewLarge.hide();
		    ...
		    addView(mMediaPlayerControllerViewSmall, mMediaPlayerControllerViewSmallParams);
		    mMediaPlayerControllerViewSmall.hide();
		}
		...
		post(new Runnable() {
		    ...
		    try {
		            ...
		            mLayoutParamFullScreenMode = (android.view.ViewGroup.LayoutParams) constructor.newInstance(
        ...
        if (event.getKeyCode() == KeyEvent.KEYCODE_BACK && event.getAction() == KeyEvent.ACTION_DOWN) {       
            ...
            if (MediaPlayerUtils.isFullScreenMode(mPlayMode)) {
                if (mLockMode) {
                    ...
                    mMediaPlayerController.onRequestPlayMode(MediaPlayMode.PLAYMODE_WINDOW);
                }
                return true;
            }
            else if (MediaPlayerUtils.isWindowMode(mPlayMode)) {
    ...
    private void doScreenOrientationRotate(int screenOrientation) {
        ...
        case ORIENTATION_LANDSCAPE_REVERSED:
            ...
            if (mDeviceNavigationBarExist && mFullScreenNavigationBarHeight <= 0
	...
	private void updateVideoInfo2Controller() {
		...
		mMediaPlayerControllerViewLarge.updateVideoQualityState(MediaPlayerVideoQuality.SD);
    ...
    IMediaPlayer.OnCompletionListener mOnCompletionListener = new IMediaPlayer.OnCompletionListener() {
        ...
        public void onCompletion(IMediaPlayer mp) {
        	 ...
        	 mMediaPlayerControllerViewLarge.hide();
        	 mMediaPlayerControllerViewSmall.hide();
        	 mMediaPlayerCoverView.updateCoverMode(MediaPlayerEventActionView.EVENT_ACTION_VIEW_MODE_COMPLETE, null);
...
//        	Log.i("guoli", "precent :" + percent);
    ...
    IMediaPlayer.OnErrorListener mOnErrorListener = new IMediaPlayer.OnErrorListener() {
        ...
        public boolean onError(IMediaPlayer mp, int what, int extra) {
        	...
        	mMediaPlayerControllerViewLarge.hide();
            mMediaPlayerControllerViewSmall.hide();
            mMediaPlayerCoverView.updateCoverMode(MediaPlayerEventActionView.EVENT_ACTION_VIEW_MODE_ERROR, what + "," + extra);
            mMediaPlayerCoverView.show();
        	return true;
        }
    };
    ...
    private MediaPlayerBaseControllerView.MediaPlayerController mMediaPlayerController = 
    		new MediaPlayerBaseControllerView.MediaPlayerController() {
				...
				public void seekTo(long pos) {
					if (canSeekBackward() && canSeekForward()) {
						...
						Toast.makeText(getContext(), "current is real stream, seek is unSupported !", Toast.LENGTH_SHORT).show();
					}
		        }
				...
				public void onRequestPlayMode(int requestPlayMode) {
		            if (mPlayMode == requestPlayMode)   return;
		            if (mLockMode)  return;
		            // 请求全屏模式
		            if (MediaPlayerUtils.isFullScreenMode(requestPlayMode)) {
				...
				public void onBackPress(int playMode) {
					Log.i("guoli", "========playerview back pressed ==============playMode :" + playMode + ", mPlayerViewCallback is null " + (mPlayerViewCallback==null));
