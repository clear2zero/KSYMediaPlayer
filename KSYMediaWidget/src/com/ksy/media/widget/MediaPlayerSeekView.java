...
public class MediaPlayerSeekView extends RelativeLayout {
    ...
    private void init(Context context){
        ...
        View rootView = LayoutInflater.from(mContext).inflate(R.layout.media_player_seek_view, null);
...
//        }
//        Log.d("onGestureSeekChange deltaSeekDistance,mTotalDeltaSeekDistance = " + deltaSeekDistance+"-"+mTotalDeltaSeekDistance);
        if (Math.abs(mTotalDeltaSeekDistance) >= mMinSeekDistance) {
            ...
            if(mSeekPosition > mInitPosition){
                if(mSeekState != SEEK_STATE_FORWARD){
                    ...
                    if(mTvSeekStateRewind.getVisibility() == View.VISIBLE) mTvSeekStateRewind.setVisibility(View.INVISIBLE);
                    if(mTvSeekStateFastForward.getVisibility() != View.VISIBLE) mTvSeekStateFastForward.setVisibility(View.VISIBLE);
                }
            }
            // 快退
            else{
                if(mSeekState != SEEK_STATE_BACK){
                    ...
                    if(mTvSeekStateFastForward.getVisibility() == View.VISIBLE) mTvSeekStateFastForward.setVisibility(View.INVISIBLE);
                    if(mTvSeekStateRewind.getVisibility() != View.VISIBLE) mTvSeekStateRewind.setVisibility(View.VISIBLE);
                }
            }
...
//            }
    ...
    public void onGestureSeekBegin(long currentPosition, long totalPosition){
        ...
        mTvSeekTotalPosition.setText(MediaPlayerUtils.getVideoDisplayTime(totalPosition));
